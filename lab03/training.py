...
def _build_sequential_model(model_type: str, vocab_size: int, num_classes: int):
...
def train_sequential_model(
    ...
    X_train = texts_to_padded_sequences(tokenizer, train_texts, max_len=max_len)
    X_val = texts_to_padded_sequences(tokenizer, val_texts, max_len=max_len)
    ...
    model = _build_sequential_model(normalized_model, vocab_size, len(label_encoder.classes_))
    ...
    artifact_paths = save_artifacts(model, tokenizer, label_encoder, normalized_model, dataset_name)
